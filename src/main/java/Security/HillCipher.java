...
public class HillCipher {
    ...
    public List<Integer> analyse3By3Key(List<Integer> plainText, List<Integer> cipherText) {
        // Students should complete this part
        int n = 3;
        if (plainText.size() < n * n || cipherText.size() < n * n) throw new InvalidAnalysisException();

        List<Integer> plainMatrix  = new ArrayList<>();
        List<Integer> cipherMatrix = new ArrayList<>();

        for (int row = 0; row < n; row++) {
            for (int col = 0; col < n; col++) {
                plainMatrix.add(plainText.get(col * n + row));
                cipherMatrix.add(cipherText.get(col * n + row));
            }
        }

        List<Integer> inversePlain = invertMatrix(plainMatrix, n);
        return multiplyMatricesMod26(cipherMatrix, inversePlain, n);
    }
}
